...
def prepare_word_data(loader, model, phoneme_to_id, limit=None):
    """
    ...
    for i, (word, phonemes_with_stress) in enumerate(word_list):
        if i % 1000 == 0:
            print(f"Processing word {i}/{len(word_list)}...")
...
def main():
    ...
    # Prepare data
    print("\n[3/4] Preparing word data...")
    words_data = prepare_word_data(loader, model, phoneme_to_id, limit=args.limit)
