...
2. **Psycholinguistic Norms** - 12 properties from 6 major research datasets
...
PhonoLex integrates psycholinguistic norms from 6 major research datasets:
...
**Vocabulary Filtering (v2.3)**: Only words with **frequency + at least one additional psycholinguistic norm** (concreteness, AoA, imageability, familiarity, or VAD) are included. This reduces vocabulary from 48K → 18K words (63% reduction) while ensuring high-quality data for research and clinical applications.
...
│               ├── word_metadata.json      # 18K words with all properties
...
- **18K words** with comprehensive psycholinguistic norms
...
# Automatically filters: frequency + at least one psycholinguistic norm (63% reduction: 48K → 18K words)
```
...
# This creates:
# - word_metadata.json - 18K words with all properties and psycholinguistic norms
...
- `word_metadata.json`: 18K words with phonological properties and psycholinguistic norms
- `embeddings.json.gz`: Phoneme-sequence syllable structures (v2.3)
- `minimal_pairs.json.gz`: Precomputed minimal pairs
- `phoneme_features.json.gz`: PHOIBLE features for 94 English phonemes
...
- **Phase 1**: 94 English phonemes (extracted from Phoible)
- **Phase 2**: 94 phonemes with continuous vectors
- **Phase 3**: 18K words with phoneme-sequence syllable structures (v2.3)
- **Client-side data**: 18K words with comprehensive psycholinguistic norms
- **Dialect**: General American English (CMU primary pronunciations only)
