...
def load_phoible_init(phoneme_to_id, d_model=128):
    """Initialize with Layer 2 normalized vectors (76-dim → d_model)"""
    print("=" * 70)
    print("INITIALIZING FROM LAYER 2 EMBEDDINGS")
    print("=" * 70)

    # Load Layer 2 embeddings (pre-computed 76-dim normalized vectors)
    import pickle
    layer2_path = Path('embeddings/layer2/normalized_76d.pkl')

    if not layer2_path.exists():
        raise FileNotFoundError(
            f"Layer 2 embeddings not found at {layer2_path}\n"
            "Please run: python scripts/compute_layer2_normalized_vectors.py"
        )

    with open(layer2_path, 'rb') as f:
        phoible_features = pickle.load(f)

    print(f"✓ Loaded {len(phoible_features)} phoneme embeddings from Layer 2")
...
def main():
    ...
    # Save
    Path('models/layer3').mkdir(parents=True, exist_ok=True)
    torch.save({
        'model_state_dict': model.state_dict(),
        'phoneme_to_id': phoneme_to_id,
        'max_length': dataset.max_length
    }, 'models/layer3/model.pt')

    print("✓ Model saved: models/layer3/model.pt\n")

    # Evaluate on CPU (MPS has issues with attention masks)
    print("=" * 70)
    ...
    def get_seq(word):
        ...
        with torch.no_grad():
            _, _, contextual = model(
                torch.LongTensor([padded]).to(eval_device),
                torch.LongTensor([mask]).to(eval_device)
            )
