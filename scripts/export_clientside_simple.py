...
def main():
    ...
    emb_path = (
        project_root / "embeddings/layer4/syllable_embeddings_filtered_quantized.pt"
    )
    ...
    ipa_to_arpa = {
        v: k for k, v in loader.arpa_to_ipa.items() if k.rstrip("012") == k
    }  # Base forms only
    print("  ✓ Loaded ARPAbet mappings")
    ...
    print("  ✓ arpa_to_ipa.json (mapping reference)")
    ...
    embeddings_json = {}
    for word, syllable_arrays in tqdm(
        checkpoint["quantized_embeddings"].items(), desc="  Converting embeddings"
    ):
