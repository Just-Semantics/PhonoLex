...
class PhonologyDataset(Dataset):
    ...
    def __init__(self, loader: EnglishPhonologyLoader, mask_prob: float = 0.15):
        ...
        self.phoneme_to_id.update(
            {p: i + 4 for i, p in enumerate(sorted(all_phonemes))}
        )
    ...
    def _create_contrastive_pairs(self):
        ...
        for pair in self.loader.morphology:
            if (
                pair.lemma in self.loader.lexicon
                and pair.inflected in self.loader.lexicon
            ):
    ...
    def get_contrastive_batch(self, batch_size):
        ...
        positive_batch = random.sample(
            self.positive_pairs, min(batch_size // 2, len(self.positive_pairs))
        )
        negative_batch = random.sample(
            self.negative_pairs, min(batch_size // 2, len(self.negative_pairs))
        )
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, phoneme_embeddings, attention_mask):
        ...
        # Weighted sum
        pooled = torch.sum(
            phoneme_embeddings * weights.unsqueeze(-1), dim=1
        )  # (batch, d_model)
...
class PhonoLexBERT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        )  # (batch, seq_len, num_phonemes)

        # Word-level embeddings via attention pooling
        word_embeddings = self.attention_pooling(
...
def contrastive_loss(emb1, emb2, labels, temperature=0.07):
    ...
    loss = nn.functional.binary_cross_entropy_with_logits(
        similarity, labels, reduction="mean"
    )
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            mlm_loss = mlm_criterion(
                mlm_logits.view(-1, mlm_logits.size(-1)), target_ids.view(-1)
            )
            ...
            if batch_idx % 2 == 0:
                ...
                if len(pairs) > 0:
                    ...
                    masks1 = []
                    for seq in seqs1:
                        padded = seq + [dataset.phoneme_to_id[dataset.PAD]] * (
                            max_len - len(seq)
                        )
            ...
            accuracy = (
                100.0 * total_correct / total_predictions
        ...
        print(
            f"Epoch {epoch+1}/{num_epochs} - MLM Loss: {avg_mlm_loss:.4f}, "
