...
def load_metadata(metadata_path: str = None) -> dict:
    ...
    if metadata_path is None:
        metadata_path = (
            project_root / "data" / "norms" / "cognitive_graph_v7.metadata.json"
        )
        ...
        summary_data.append(
            {
                ...
                "source": (
                    feature_examples[feature][0]["source"]
                    if feature_examples[feature]
                    else "unknown"
                ),
            }
        )
...
def find_words_with_all_features(
    ...
    print(
        f"  Words with ALL features: {len(complete_rows)} ({len(complete_rows)/len(df)*100:.1f}%)"
    )
...
def main():
    ...
    for feature in key_features:
        if feature in norms_df.columns:
            ...
            print(
                f"  {feature:20s}: {count:6d} words ({count/len(norms_df)*100:5.1f}%)"
            )
        else:
    ...
    complete_output_path = (
        project_root / "data" / "norms" / "psycholinguistic_norms_complete.csv"
    )
    complete_words.to_csv(complete_output_path, index=False)
    print(
        f"✓ Saved complete norms ({len(complete_words)} words) to: {complete_output_path}"
    )
